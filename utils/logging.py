...
def tensorboard_map_log(name, val_map_05, val_map, writer, epoch):
    writer.add_scalars(
        name, {"mAP@0.5": val_map_05[-1], "mAP@0.5_0.95": val_map[-1]}, epoch
    )
        ...
        df = pd.DataFrame(
            {
                ...
                "bbox regression": bbox_reg_list[-1],
            }
        )

    if loss_box_reg_list:
        df = pd.DataFrame(
            {
                "epoch": int(epoch + 1),
                "map_05": [float(stats[0])],
...
def wandb_log(
    ...
    wandb.log({"val_map_05_95": val_map})
    wandb.log({"val_map_05": val_map_05})
    ...
    if len(val_pred_image) > 2 and len(val_pred_image) <= 8:
        ...
        for i in range(len(val_pred_image) - 1):
            log_image = cv2.hconcat(
                [log_image, overlay_on_canvas(bg, val_pred_image[i + 1])]
            )
        wandb.log({"predictions": [wandb.Image(log_image)]})
...
class LogJSON:
    def __init__(self, output_filename):
        ...
        if not os.path.exists(output_filename):
            ...
            with open(output_filename, "w") as file:
                json.dump(
                    {"images": [], "annotations": [], "categories": []}, file, indent=4
                )
    ...
    def update(self, image, file_name, boxes, labels, classes):
        ...
        self.coco_data["categories"] = [
            {"id": cat_id, "name": classes[cat_id]} for cat_id in self.categories
        ]
